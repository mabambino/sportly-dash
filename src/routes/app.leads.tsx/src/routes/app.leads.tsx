    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Track prospects and trial conversions</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button className="bg-gradient-hero"><Plus className="mr-2 h-4 w-4" /> Add lead</Button>
          </DialogTrigger>
          <AddLeadDialog
            clubId={club!.id}
            onDone={() => { setOpen(false); qc.invalidateQueries({ queryKey: ["leads"] }); }}
          />
        </Dialog>
      </div>
                  ...
                  <Card key={lead.id} className="p-3 space-y-2">
                    ...
                    <div className="flex gap-1.5 pt-1">
                      {NEXT_STATUS[lead.status] && (
                        <Button size="sm" variant="outline" className="h-7 text-xs flex-1" onClick={() => advance(lead)}>
                          Move to {NEXT_STATUS[lead.status]}
                        </Button>
                      )}
                      {lead.status !== "lost" && lead.status !== "converted" && (
                        <Button size="sm" variant="ghost" className="h-7 text-xs text-destructive" onClick={() => markLost(lead.id)}>
                          Lost
                        </Button>
                      )}
                    </div>
    ...
    <DialogContent>
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div><Label>Email</Label><Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="jane@email.com" /></div>
          <div><Label>Phone</Label><Input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+1 555 000" /></div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div><Label>Sport</Label><Input value={sport} onChange={(e) => setSport(e.target.value)} placeholder="Football" /></div>
          <div><Label>Trial date</Label><Input type="date" value={trialDate} onChange={(e) => setTrialDate(e.target.value)} /></div>
        </div>
        <div><Label>Notes</Label><Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Source, context…" rows={3} /></div>
